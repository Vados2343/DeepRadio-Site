...
class StatsView extends HTMLElement {
  ...
  connectedCallback() {
    ...
    window.addEventListener('languageChanged', () => this.updateTranslations());
  }

  disconnectedCallback() {
    this.stopRealtimeUpdates();
    window.removeEventListener('languageChanged', () => this.updateTranslations());
  }
  ...
  attachEventListeners() {
    ...
    // Поиск
    const searchInput = this.shadowRoot.getElementById('track-search');
    if (searchInput) {
      let searchTimeout;
  ...
  getAllStats() {
    ...
    stats.sessions.forEach(session => {
      if (session.timestamp >= today) {
        todayTime += session.time;
      }
      if (session.timestamp >= weekAgo) {
        weekTime += session.time;
      }
    });
    ...
    return {
      ...
      times: {
        today: todayTime,
        week: weekTime,
        total: stats.totalTime + (currentSession ? Date.now() - currentSession.startTime : 0)
      }
    };
  }
  ...
  updateCurrentSession() {
    ...
    if (!currentSession || !store.isPlaying) {
    ...
    this.shadowRoot.getElementById('current-session-icon').src = iconUrl;
    this.shadowRoot.getElementById('current-session-station').textContent = currentSession.stationName;
    
    if (currentSession.track) {
      const trackText = currentSession.track.artist 
        ? `${currentSession.track.artist} - ${currentSession.track.song}`
        : currentSession.track.song;
      this.shadowRoot.getElementById('current-session-track').textContent = trackText;
    } else {
      this.shadowRoot.getElementById('current-session-track').textContent = '';
    }

    this.shadowRoot.getElementById('current-session-time').textContent = 
      `${t('stats.listening')}: ${this.formatTime(sessionTime)}`;
  }
  ...
  renderCalendar() {
    ...
    this.shadowRoot.getElementById('calendar-current').textContent = 
    ...
    weekdays.forEach(day => {
      ...
      elem.className = 'calendar-weekday';
    ...
    for (let i = startDay - 1; i > 0; i--) {
      ...
      const elem = this.createCalendarDay(new Date(year, month - 1, day), true, dailyStats);
      grid.appendChild(elem);
    }
    ...
    for (let day = 1; day <= lastDay.getDate(); day++) {
      ...
      const elem = this.createCalendarDay(date, false, dailyStats);
      grid.appendChild(elem);
    }
  ...
  createCalendarDay(date, otherMonth, dailyStats) {
    const elem = document.createElement('div');
    ...
    numberElem.textContent = date.getDate();
    elem.appendChild(numberElem);

    if (dayData) {
      const timeElem = document.createElement('div');
      timeElem.className = 'calendar-day-time';
      timeElem.textContent = this.formatShortTime(dayData.time);
  ...
  showDayStats(dateStr) {
    ...
    if (!dayStats || dayStats.sessions.length === 0) {
      container.innerHTML = `
  ...
  updateGenreChart(genres) {
    ...
    container.innerHTML = sorted.slice(0, 10).map(([genre, time]) => {
  ...
  updateTranslations() {
    // Обновление всех переводов после смены языка
    this.shadowRoot.querySelectorAll('[data-i18n]').forEach(elem => {
      const key = elem.getAttribute('data-i18n');
      elem.textContent = t(key);
    });

    this.shadowRoot.querySelectorAll('[data-i18n-placeholder]').forEach(elem => {
      const key = elem.getAttribute('data-i18n-placeholder');
      elem.placeholder = t(key);
    });

    // Перерисовка календаря и списков
    this.renderCalendar();
    this.loadStats();
  }
